...
import { receptionEscalateMaxConfidence, receptionRouteMinCompleteness, receptionRouteMinConfidence, slackEscalationChannelLabel, } from './reception-thresholds.js';
let receptionAgentSingleton;
...
function buildSpecialistHandoffPrompt(userPrompt, decision) {
    return [
...
async function maybeNotifyLowConfidence(decision, ctx) {
...
function shouldAutoRoute(decision) {
...
export async function runReceptionAgentTurn(request) {
    ...
    if (!parsed.success) {
        ...
        return {
            success: false,
            message: '受付エージェントの構造化出力を検証できませんでした。会話を続けるか、別の言い回しでもう一度お試しください。',
            details: {
                traceId: context.traceId,
                sessionId: context.sessionId,
                channel: 'reception',
