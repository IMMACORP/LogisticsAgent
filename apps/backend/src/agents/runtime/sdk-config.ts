...
import {
  ...
  getOpenAIOrganizationId,
  getOpenAIProjectId,
  getTrimmedOpenAIApiKey,
  isProjectScopedApiKey,
} from '../../lib/openai/openai-env.js';
...
export function assertOpenAIKeyPresent(): void {
  ...
  if (isProjectScopedApiKey(key) && !getOpenAIProjectId()) {
    throw new Error(
      'OPENAI_PROJECT_ID is required when using a project API key (sk-proj-*). ' +
        'Set it in apps/backend/.env (Project ID from OpenAI dashboard).',
    );
  }
}

export function resolveAgentsModel(): string | undefined {
 ...
 */
export function configureOpenAIAgentsSdk(): void {
...
export function logOpenAISetupHint(): void {
