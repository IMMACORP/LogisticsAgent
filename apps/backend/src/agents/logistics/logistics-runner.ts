...
import {
  logisticsOperationOutputSchema,
  type LogisticsOperationOutput,
} from '../../schemas/logistics/logistics-operation.schemas.js';
...
import {
  logisticsEscalateMaxConfidence,
  logisticsRetryBaseDelayMs,
  logisticsRunMaxAttempts,
} from './logistics-thresholds.js';

let logisticsAgentSingleton: ReturnType<typeof createLogisticsOperationSupportAgent> | undefined;

function getLogisticsOperationAgentSingleton() {
  logisticsAgentSingleton ??= createLogisticsOperationSupportAgent(
...
async function maybeProgrammaticEscalation(
  out: LogisticsOperationOutput,
  ctx: AgentRunContext,
  usedNotifySlack: boolean,
): Promise<boolean> {
...
export async function runLogisticsOperationAgentTurn(request: AgentRequest): Promise<AgentResponse> {
  ...
  try {
    const runResult = await runWithRetries(
      async () => {
        attemptsUsed += 1;
        return runtime.run({
          agent,
