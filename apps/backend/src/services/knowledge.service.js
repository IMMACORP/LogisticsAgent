...
function locator(documentCode, chunkIndex) {
...
function lexicalScore(queryTokens, queryRaw, text) {
...
export class KnowledgeService {
    vectorSearch;
    embedder;
    constructor(vectorSearch, embedder) {
    ...
    async searchKnowledgeBase(input) {
        ...
        if (queryTokens.length === 0) {
            throw new ToolExecutionError('検索に使用できる語がクエリに含まれていません', 'INVALID_INPUT');
        }
        const warnings = [];
        const lexicalHits = [];
        ...
        if (input.includeSemantic) {
            ...
            if (this.vectorSearch.isConfigured() && this.vectorSearch.backend === 'pgvector') {
                ...
                warnings.push('セマンティック検索はローカル埋め込み類似度のプレースホルダです。本番では pgvector + 事前計算チャンク埋め込みに差し替えてください。');
                const candidates = [];
                for (const doc of knowledgeCorpusSeeds) {
                    if (input.category && doc.category !== input.category) {
                        continue;
        ...
        const semanticChunks = semanticHits.map((hit) => ({
            citation: {
                ...
                locator: locator(hit.chunk.documentCode ?? hit.chunk.documentId, hit.chunk.chunkIndex),
        ...
        if (!input.includeSemantic) {
            ...
            for (const hit of lexicalTop) {
                ...
                if (!existing) {
                    ...
                    const fusedScore = Math.max(existing.score ?? 0, hit.score ?? 0);
            ...
            merged = [...byChunk.values()].sort((a, b) => (b.score ?? 0) - (a.score ?? 0));
            merged = merged.slice(0, limit);
        }
        if (merged.length === 0) {
            throw new ToolExecutionError('該当するナレッジチャンクが見つかりません', 'NO_RESULTS', { query: input.query, category: input.category });
        }
        return {
            chunks: merged,
            retrieval: {
