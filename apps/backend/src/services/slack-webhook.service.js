...
export class SlackWebhookService {
    async send(body) {
        ...
        if (!url) {
            throw new ToolExecutionError('Slack Incoming Webhook URL が未設定です。SLACK_ESCALATION_WEBHOOK_URL または SLACK_WEBHOOK_URL を設定してください。', 'SLACK_UNAVAILABLE');
        }
        ...
        for (let attempt = 0; attempt <= maxRetries; attempt += 1) {
            ...
            try {
                ...
                const okSlackWebhook = response.ok &&
                ...
                if (!retryable || attempt === maxRetries) {
                    throw new ToolExecutionError(`Slack Webhook が失敗しました (HTTP ${response.status}): ${responseText.slice(0, 500)}`, 'SLACK_UNAVAILABLE', { status: response.status });
                }
        ...
        if (lastError instanceof Error) {
            throw new ToolExecutionError(`Slack Webhook のリトライが尽きました: ${lastError.message}`, 'SLACK_UNAVAILABLE');
        }
        throw new ToolExecutionError('Slack Webhook のリトライが尽きました', 'SLACK_UNAVAILABLE');
    }
}
