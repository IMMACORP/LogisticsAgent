...
import type { EscalationHistoryItem } from '../schemas/history/history.schemas.js';
...
export class EscalationHistoryService {
  ...
  async listEscalations(query: ListEscalationsQuery): Promise<{
  ...
  async getEscalation(escalationId: string): Promise<EscalationHistoryItem | null> {
...
export const escalationHistoryService = new EscalationHistoryService(
  new EscalationRepository(prisma),
);
