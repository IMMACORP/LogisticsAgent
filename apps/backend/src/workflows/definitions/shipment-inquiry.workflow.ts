import type { GetShipmentStatusOutput } from '@inquiry-agent/shared-types';
...
export interface ShipmentInquiryWorkflowInput {
  trackingNumber: string;
}

export function createShipmentInquiryWorkflow(): WorkflowDefinition {
  return {
    ...
    steps: [
      ...
      },
      {
        ...
        run: async (ctx) => {
          try {
            const status = await shipmentService.getShipmentStatus(
              String(ctx.state.trackingNumber),
            );
      ...
      },
      {
        ...
        timeoutMs: 2_000,
        run: async (ctx) => {
          const s = ctx.state.shipmentStatus as GetShipmentStatusOutput | undefined;
          if (!s) {
