import type { InventoryRecord } from '@inquiry-agent/shared-types';
...
export interface InventoryShortageWorkflowInput {
...
export function createInventoryShortageWorkflow(): WorkflowDefinition {
  return {
    ...
    steps: [
        ...
        run: async (ctx) => {
          ...
          if (!warehouseCode || !itemCode || !Number.isFinite(requiredQuantity) || requiredQuantity <= 0) {
            throw new WorkflowError(
              'VALIDATION',
              'warehouseCode, itemCode, and positive requiredQuantity are required',
            );
          }
      ...
      },
      {
        ...
        run: async (ctx) => {
          ...
          const alternates = (records as InventoryRecord[]).filter(
            (r) =>
              r.warehouseCode !== ctx.state.warehouseCode &&
        ...
        run: async (ctx) => {
          ...
          ctx.state.summary =
            ctx.state.severity === 'ok'
              ...
              : `在庫不足: 必要 ${required} / 可能 ${available}。代替倉庫: ${
                  (ctx.state.alternateWarehouses as unknown[])?.length ?? 0
                } 件`;
        },
      },
    ],
