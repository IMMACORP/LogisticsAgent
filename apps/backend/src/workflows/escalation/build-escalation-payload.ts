import type { NotifySlackPriority } from '@inquiry-agent/shared-types';

import {
  structuredWorkflowEscalationSchema,
...
import type { TriggerEvaluationResult } from './evaluate-triggers.js';

export function buildEscalationTitle(activeTriggers: string[]): string {
...
export function buildStructuredEscalationPayload(params: {
  ...
  const payload: StructuredWorkflowEscalation = {
    schemaVersion: '1.1',
    idempotencyKey: params.input.idempotencyKey,
    triggers: params.evaluation.activeTriggers,
    priority: params.evaluation.priority as NotifySlackPriority,
