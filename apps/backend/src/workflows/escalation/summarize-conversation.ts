 ...
 */
export function summarizeConversationForEscalation(
  input: Pick<EscalationWorkflowInput, 'context'>,
  ...
  if (input.context.shipment) {
    lines.push(
      '',
      '--- 配送コンテキスト ---',
      JSON.stringify(input.context.shipment, null, 0),
    );
  }
  if (input.context.toolFailure) {
    lines.push(
      '',
      '--- ツール失敗 ---',
      `${input.context.toolFailure.toolName}: ${input.context.toolFailure.message}`,
    );
  }
  if (input.context.retryFailure) {
    lines.push(
      '',
      '--- リトライ枯渇 ---',
      `${input.context.retryFailure.operation} (${input.context.retryFailure.attempts} 回)`,
    );
  }
