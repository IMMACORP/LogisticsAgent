...
function isMissingShipmentData(ctx) {
...
export function evaluateEscalationTriggers(requested, context) {
    ...
    if (activeTriggers.includes('repeated_retry_failure') ||
...
export function assertTriggersSubstantiated(requested, context) {
    const result = evaluateEscalationTriggers(requested, context);
    if (result.activeTriggers.length === 0) {
        throw new Error(`No escalation triggers substantiated for: ${requested.join(', ')}. Provide matching context.`);
    }
    return result;
}
