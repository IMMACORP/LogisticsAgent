...
import { configureOpenAIAgentsSdk, isOpenAIAgentsSdkEnabled, logOpenAISetupHint, } from './agents/runtime/sdk-config.js';
import { agentRouter } from './routes/agent';
import { chatRouter } from './routes/chat.js';
import { healthRouter } from './routes/health';
import { workflowsRouter } from './routes/workflows.js';
import { openApiApp } from './openapi/router.js';
import { maskOpenAIKey, validateOpenAIApiKey, } from './lib/openai/validate-api-key.js';
void (async () => {
    if (!isOpenAIAgentsSdkEnabled()) {
        return;
    }
    console.log(`[startup] OPENAI_API_KEY loaded: ${maskOpenAIKey(getTrimmedOpenAIApiKey())} (from ${path.join(backendRoot, '.env')})`);
    logOpenAISetupHint();
    try {
