import {
  ...
  describeOpenAIKeySetup,
  getTrimmedOpenAIApiKey,
  isProjectScopedApiKey,
  getOpenAIProjectId,
} from './openai-env.js';
...
export interface OpenAIKeyCheckResult {
  ok: boolean;
  message: string;
  /** Last 4 characters — safe to log for confirming which key loaded */
  keySuffix?: string;
}

export function maskOpenAIKey(key: string | undefined): string {
  if (!key) {
    return '(not set)';
  }
 ...
 */
export async function validateOpenAIApiKey(): Promise<OpenAIKeyCheckResult> {
  ...
  if (isProjectScopedApiKey(key) && !getOpenAIProjectId()) {
    return {
      ok: false,
      message:
        'Project API key (sk-proj-*) requires OPENAI_PROJECT_ID=proj_... in apps/backend/.env. ' +
  ...
  try {
    ...
    if (response.status === 401) {
      return {
        ok: false,
        message:
          'OpenAI returned 401. If the key works elsewhere, set OPENAI_PROJECT_ID (and OPENAI_ORG_ID if needed) ' +
