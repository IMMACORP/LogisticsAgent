...
function severityToEmoji(severity: EscalationSeverity): string {
  switch (severity) {
    case 'info':
...
export interface EscalationSlackMessage {
  /** Plain-text fallback for notifications / mobile */
  text: string;
  /** Block Kit payload for Slack */
  blocks: unknown[];
  /** Canonical escalation record for logs and downstream consumers */
  escalation: StructuredEscalationPayload;
}
...
export function createEscalationMessage(
  ...
  const mentionLine =
    escalation.mentionUsers.length > 0
