...
} from '../../schemas/shipment/shipment.schemas';
...
export const getShipmentStatusAgentTool = tool({
  ...
  description:
    '送り状番号（trackingNumber）を指定して、配送ステータス・現在地・ETA・遅延理由を取得します。物流オペレーションエージェント向け。',
...
export const searchShipmentHistoryAgentTool = tool({
  ...
  description:
    '送り状番号・荷主名・ステータス・期間で配送履歴を検索します。過去の出荷・着荷状況の照会に使用します。',
...
export const searchDeliveryIssueAgentTool = tool({
  ...
  description:
    '配送遅延（DELAYED）や出荷キャンセル（CANCELLED）などの配送トラブルを検索します。遅延理由・現在地・ETAを返します。',
