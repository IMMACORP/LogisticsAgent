...
import {
  checkStockAvailabilityInputSchema,
  reserveInventoryToolParametersSchema,
  searchInventoryToolParametersSchema,
} from '../../schemas/inventory/inventory.schemas';
...
export const searchInventoryAgentTool = tool({
  ...
  description:
    '倉庫コード・品目コード・品目名で在庫を検索します。在庫数・引当済数・引当可能数を返します。',
...
export const checkStockAvailabilityAgentTool = tool({
  ...
  description:
    '指定倉庫・品目の引当可能数が要求数量を満たすか照会します（出荷前の在庫確認に使用）。',
...
export const reserveInventoryAgentTool = tool({
  ...
  description:
    '指定倉庫・品目の在庫を引当します。トランザクション内で在庫ロック（プレースホルダ）を取得し、引当可能数を検証してから reserved_quantity を更新します。',
